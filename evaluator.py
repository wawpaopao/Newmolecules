...
class Evaluator():
    ...
    def eval(self,model,pairs,labels,epoch):
        def run_model(chem_repr, prot_repr, label, evaluate=True):
            ...
            if evaluate:
                f1, auc, aupr,accu,recall_score = evaluate_binary_predictions(batch_labels, logits)
            else:
        ...
        if len(pairs)<= self.batch:
            #sample size smaller than a batc
            print(len(pairs))
            chem_repr, prot_repr = get_repr_from_pairs_3(pairs)
...
def evaluate_binary_predictions(label,predprobs):
    ...
    prec, reca, thresholds = metrics.precision_recall_curve(label, probs, pos_label=1)
