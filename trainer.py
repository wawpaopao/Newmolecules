...
class Trainer():
    def __init__(self, model=None,
            epoch=150,batch_size=72,ckpt_dir='./temp/',
                 optimizer='adam',l2=0.002, lr=0.005, scheduler='cosineannealing',
    ...
    def train(self, edges, train_evaluator, dev_edges, dev_evaluator,test_edges, test_evaluator, checkpoint_dir):
        ...
        for epoch in range(1, self.train_epoch + 1):
            ...
            for batch_ in range(batch_per_epoch):
                ...
                if len(choices)==batch_size:
                    ...
                    #print(batch_logits)
                    #batch_labels=batch_bels.unsqueeze(1)
            ...
            model.eval()
            dev_labels = torch.tensor(dev_labels).cuda()
            dev_chem_repr, dev_prot_repr = get_repr_from_pairs_3(dev_pairs)
            dev_chem_embed = load_from_mol_tuple(dev_chem_repr)
            print(dev_chem_embed)
            if isinstance(dev_chem_embed, Variable) and torch.cuda.is_available():
                dev_chem_embed = dev_chem_embed.cuda()
            if isinstance(dev_prot_repr, Variable) and torch.cuda.is_available():
                dev_prot_repr = dev_prot_repr.cuda()
            dev_input = {'protein': dev_prot_repr, 'ligand': dev_chem_embed}
            loss_fn2 = torch.nn.BCEWithLogitsLoss()
            with torch.no_grad():
                dev_logits = model(dev_input)
            dev_logits = dev_logits.squeeze(dim=-1)
            
            valid_loss = loss_fn2(dev_logits,dev_labels)
            probs = dev_logits.cpu().numpy()
            probs = 1/(1+np.exp(-probs))
        
            predclass = probs>=0.5
            dev_labels = dev_labels.cpu().numpy()
            temp = predclass - dev_labels
            idx = np.where(temp!=0)
            print(idx)
            ...
            tb_writer.add_scalar('devacc',devmetrics[3],epoch)
            tb_writer.add_scalar('trainacc',trainmetrics[3],epoch)
            ...
            #train_loss.append(trainmetrics[5])
            ...
            #test_loss.append(testmetrics[5])
            ...
            #np.save(checkpoint_dir+'train_loss.npy',train_loss)
            ...
            #np.save(checkpoint_dir + 'dev_loss.npy', dev_loss)
            ...
            #record_dict['train_loss'].append(trainmetrics[5])
            ...
            #record_dict['dev_loss'].append(devmetrics[5])
            target_metric = devmetrics[3] #accu for binary
        ...
        loss_valid =torch.Tensor(loss_valid)
        loss_valid = loss_valid.cpu().numpy()
        
                                                
                            
        plt.plot(x,loss_train,'ro-',label='Training loss')
        plt.plot(x,loss_valid,'co-',label='Validatin loss')
        plt.title('Training and validation loss')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        plt.show()
        plt.savefig(checkpoint_dir+'loss_png')
